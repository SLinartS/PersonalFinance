<template>
    <div class="balance-change balance-change--operation-add">
        <div class="balance-change__header balance-change__header--3fr">
            <img :src="deleteImg" class="data-title__button" @click="togglePopupCategoryDelete" />
        ...
        <form>
            <div class="change__block">
                ...
                <input type="text" class="change__block-input" @input="e => description = e.target.value" />
                ...
                <select name id class="change__block-input" v-model="storageType">
                    <CategorySelectOption v-for="select in currentData['accounts']" :key="select['id']"
                        :id="select['id']" :title="select['title']" :amount="select['amount']" :type="select['type']">
                    </CategorySelectOption>
                </select>
                <h2 class="change__block-title">Время</h2>
                <input type="datetime" class="change__block-input change__block-input--datetime" :value="currentTime" />
            </div>
            <div class="balance-change__button-section">
                <button type="button" class="button button--cancel balance-change__button"
                    @click="togglePopupOperationAdd">Отмена</button>
                <button type="button" @click="addOperationItem" class="button button--save balance-change__button"
...
export default {
    ...
    methods: {
        ...
        togglePopupCategoryDelete() {
            this.$store.commit("togglePopupOperationAdd", { status: false });
            this.$store.commit("togglePopupOperationDelete", { status: true, categoryId: this.currentData["id"], typeAction: this.currentData["type"], typeBlock: "category" });
        },
        addOperationItem() {
